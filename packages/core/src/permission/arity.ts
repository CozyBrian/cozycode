/**
 * Command-prefix arities (ported from opencode).
 *
 * Maps a command prefix to the number of leading tokens that make up the
 * "human-understandable command" — used to derive an "always allow" pattern
 * from a concrete shell command (e.g. `git commit -m x` → prefix `git commit`
 * → always-pattern `git commit *`). Flags never count as tokens; longest
 * matching prefix wins; unknown commands default to their first token.
 */
